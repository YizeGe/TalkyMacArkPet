...
> **解决方法（任选其一）：**
> - **方法一（常规系统设置）**：
>   1. 首次双击打开 App 会弹出报错，此时点击“取消”或“好”。
>   2. 打开 Mac 的 **系统设置 (System Settings)** -> **隐私与安全性 (Privacy & Security)**。
>   3. 往下划，找到安全性这一栏，你会看到“已阻止使用 MacArkPet，因为来自身份不明的开发者”。点击旁边的 **仍要打开 (Open Anyway)** 按钮。
>   4. 输入你的 Mac 开机密码，再次点击弹窗里的“打开”即可。
> - **方法二（终端一键放行）**：如果系统设置里没有出现“仍要打开”按钮，请打开“终端 (Terminal)” App，复制并粘贴以下命令后回车：
>   ```bash
...
**1. 源码编译与运行**
```bash
git clone https://github.com/YizeGe/TalkyMacArkPet.git
cd TalkyMacArkPet
